...
bool draw::surface::read(const char* url, color* pallette, int need_bpp) {
	...
	if(!pin)
		return false;
	auto result = false;
	auto push_pallette = output_pallette; output_pallette = pallette;
...
static void change_pallette(color* pi, int count) {
...
static struct bmp_bitmap_plugin : public draw::surface::plugin {
	...
	bool decode(unsigned char* output, int output_bpp, const unsigned char* input, unsigned input_size) override {
		...
		if(output_pallette && pi->color_used > 0 && pi->bpp == 8) {
			memcpy(output_pallette, ppal, pi->color_used * sizeof(color));
			//change_pallette(output_pallette, pi->color_used);
		}
		color e;
		for(int y = 0; y < height; y++) {
			unsigned char* d = output + y * output_scanline;
			unsigned char* s = pb + ((pi->height < 0) ? y : (pi->height - y - 1))*input_scanline;
			if(input_bpp == output_bpp && input_scanline == output_scanline)
				memcpy(d, s, input_scanline);
			else {
				for(int x = 0; x < width; x++) {
					e.read(s, x, input_bpp, ppal);
					e.write(d, x, output_bpp, ppal, -1);
				}
			}
		}
		return true;
